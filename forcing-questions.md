...
Ask them **one at a time.** Push on each one **at least twice** before accepting an answer as locked. Comfort means the founder hasn't gone deep enough — and because the founder is non-technical, they will sometimes try to wave you past a fuzzy customer answer by gesturing toward a technical solution. Don't let them. Stay on the business side.

## Smart routing by stage

You don't always need all six. Match to where they are:

| Stage | Ask |
| --- | --- |
| Pre-product (idea stage, no users) | Q1, Q2, Q3 |
| Has users (people using, not paying) | Q2, Q4, Q5 |
| Has paying customers | Q4, Q5, Q6 |

**Smart-skip rule:** if the user's prior answers already cover a later question, skip it. Only ask questions whose answers aren't yet clear.

## Stricter pass criteria

A question is "passed" only when **all three** are true:

1. The answer is specific (a name, a number, a behavior, a quote — not a category).
2. The answer is grounded in something the founder has observed or done, not something they've assumed.
...
**Ask:**
> What's the smallest possible thing that one specific customer would pay you for this week — not after you build the platform, not after your co-founder ships v1, but this week?

**Push until you hear:** One feature. One workflow. Maybe a weekly email. Maybe a single automation. Maybe a manual service you provide yourself. They should be able to describe something ship-able in days, not months, that someone would actually pay for.
...
### Two ways to ship the wedge in seven days

Important: in an AI-native world, "ship in 7 days" has two valid shapes. Walk the founder through both — they're not the same and the choice matters.

**Shape 1 — The no-code / concierge / manual wedge.** The founder runs the wedge by hand for the first 5-10 customers. Spreadsheets, emails, phone calls, hand-edited PDFs. The technical co-founder builds nothing in v0. This is how most product companies actually started, including most YC alums. It is *not* a failure mode — it is how you find out in 2 weeks instead of 3 months that you're building the wrong thing.

**Shape 2 — The software-factory wedge.** The technical co-founder, using AI tooling (software factory, agent fleet, software-as-code-generation), ships a real working version in 7-14 days. This is now genuinely real — see [ai-native-context.md](ai-native-context.md) for why. The thing that used to take a small team six weeks now takes one engineer with a software factory under two weeks.

**Both are valid. Different costs:**

| Shape | Cost | When it's right |
| --- | --- | --- |
| No-code / manual | Founder's time for first 10 customers; no engineering | When demand is most uncertain, or when the product is mostly a service the founder is uniquely good at |
| Software-factory | Co-founder's two weeks; some AI tokens; some technical debt to clean up later | When demand is established and the bottleneck is throughput, not learning |

**Bonus push for both shapes:** "What if the customer didn't have to do anything at all to get value? No login, no setup, no integration. What would that look like?"
...
- Which of the two shapes (no-code or software-factory) is being chosen — and why
...
> You haven't watched anyone use this yet. That's not a small thing. Your technical co-founder is going to build whatever you describe — and you can only describe it accurately if you've seen it in use. The first assignment from this session is to fix that.
...
Once you have evidence-based, locked answers to the relevant questions, move to **Phase 3 — Premise Challenge + Alternatives** ([premise-and-alternatives.md](premise-and-alternatives.md)).

Do not skip from questions straight to the artifact draft. The premise challenge and the forced alternatives are where you catch the assumptions that would otherwise survive into the build.
